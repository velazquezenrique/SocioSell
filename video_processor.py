...
class TokenBucket:
    """Improved rate limiter with better quota management"""
    def __init__(self, tokens_per_second=0.05, max_tokens=10):  # Reduced rate
...
class VideoProcessor:
    def __init__(self, google_api_key):
        ...
        self.rate_limiter = TokenBucket(tokens_per_second=0.05)  # Updated rate limiter
        ...
        # Configuration
        self.MAX_FRAMES_PER_VIDEO = 3  # Reduced from 5
        self.MAX_API_RETRIES = 3       # Maximum retries for API calls
        self.API_RETRY_DELAY = 10      # Base delay in seconds between retries
        self.FRAME_ANALYSIS_DELAY = 5  # Delay between frame analyses
    ...
    async def _analyze_frames(self, frames):
        """Analyze video frames with improved rate limiting"""
        descriptions = []
        frames = frames[:self.MAX_FRAMES_PER_VIDEO]  # Limit number of frames
        ...
        for frame in tqdm(frames, desc="Analyzing frames"):
                ...
                for attempt in range(self.MAX_API_RETRIES):
                    try:
                        ...
                        if description:
                            ...
                            break
                        await asyncio.sleep(2)  # Small delay between successful analyses
                    except Exception as e:
            ...
            await asyncio.sleep(self.FRAME_ANALYSIS_DELAY)  # Delay between frames
...
async def main():
    try:
        GOOGLE_API_KEY = "AIzaSyCqeQCDhqqSbhz1tcKJrvr-ixtQa3TxTKY"
